...
export default function BirthdayReport() {
  ...
  const handleDownload = async () => {
    try {
      ...
      const uri = (cacheDirectory || documentDirectory) + `Birthdays_Booth_${selectedBooth}.xlsx`;
  ...
  const renderItem = ({ item }: { item: Voter }) => (
    <View style={styles.card}>
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.subText}>Part: {item.part} | {item.mobile || "No Mobile"}</Text>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyText}>No birthdays found for these filters</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  selectorValue: { fontSize: 15, fontWeight: 'bold', color: '#333' },
  ...
  dateText: { color: '#FF6600', fontWeight: 'bold', fontSize: 13 },
